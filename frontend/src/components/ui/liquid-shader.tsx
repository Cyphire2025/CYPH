  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void mainImage(out vec4 O, in vec2 fragCoord) {
        vec2 autoN =
          vec2(sin(t * 0.23), cos(t * 0.19)) * 0.22 +
          vec2(sin(t * 0.47), cos(t * 0.41)) * 0.08;
        vec2 uv = (fragCoord - 0.5 * iResolution.xy) / iResolution.y;
        uv *= m(0.15 * sin(t * 0.25));
        uv *= 1.15;
        uv += autoN * 0.18;
        vec3 col = vec3(0.0);
        float d = 2.2;
        ...
        for (int i = 0; i <= 7; i++) {
          ...
          ro.xy += autoN * vec2(1.3, -1.05);
          vec3 p = ro + normalize(vec3(uv, -1.35)) * d;
          float rz = map(p);
          float f = clamp((rz - map(p + 0.1)) * 0.5, -0.1, 1.0);

          vec3 base = hasActiveReminders
            ? vec3(0.05,0.2,0.5) + vec3(4.3,2.2,5.2)*f
            : hasUpcomingReminders
            ? vec3(0.05,0.3,0.1) + vec3(2.2,5.2,1.2)*f
            : vec3(0.1,0.3,0.4) + vec3(5.2,2.7,3.2)*f;

          col = col * base + smoothstep(2.6, 0.0, rz) * 0.72 * base;
          d += min(rz, 1.0);
        }
